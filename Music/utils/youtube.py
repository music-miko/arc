...
# Try to import API config; if missing, we'll just fall back to yt-dlp
try:
...
def _extract_video_id(link: str) -> str:
    ...
    Falls back to the last path component if needed.
    """
    link = link.strip()
    if "v=" in link:
...
async def download_song_api(link: str) -> str | None:
    """
    Use external SONG API to download audio.
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for _ in range(10):
            try:
                ...
                LOGS.error(f"[YouTube API song download failed] {e}")
                return None
        else:
            LOGS.error("YouTube API song download: max retries reached")
            return None
        ...
        try:
            ...
            LOGS.error(f"[YouTube API song download save error] {e}")
            return None


async def download_video_api(link: str) -> str | None:
    """
    Use external VIDEO API to download video.
    Returns local file path or None on failure.
    """
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for _ in range(10):
            try:
                ...
                LOGS.error(f"[YouTube API video download failed] {e}")
                return None
        else:
            LOGS.error("YouTube API video download: max retries reached")
            return None
        ...
        try:
            ...
            LOGS.error(f"[YouTube API video download save error] {e}")
            return None
...
class YouTube:
    ...
    async def download(self, link: str, video_id: bool, video: bool = False) -> str:
        """
        Used by the player. First tries API, then falls back to yt-dlp.
        ...
        # Fallback to original yt-dlp behaviour
        if video:
    ...
    async def send_song(
        ...
        """
        Used by /song or similar: send as Telegram audio/video.

        Now uses API first; if API fails or not configured, falls back to yt-dlp
        exactly like before.
        """
        ...
        try:
            ...
            if video:
                ...
                # Audio: try SONG API first
                output = await download_song_api(link)
                if output:
                    # We don't have exact duration from API, but it's optional.
